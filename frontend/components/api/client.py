...
class APIClient:
    ...
    def start_camera(self, rtsp_url):
        """Start RTSP camera"""
        response = self._post("/api/camera/start", json_data={"rtsp_url": rtsp_url})
        if response and response.status_code == 200:
            return response.json()
        elif response:
            return response.json()
    ...
    def process_frame(self, task, provider, custom_prompt=None):
        ...
        json_data = {"task": task.lower(), "provider": provider}
        if custom_prompt:
            json_data["custom_prompt"] = custom_prompt

        response = self._post("/api/llm/process", json_data=json_data)
        if response and response.status_code == 200:
            return response.json()
        elif response:
            return response.json()
